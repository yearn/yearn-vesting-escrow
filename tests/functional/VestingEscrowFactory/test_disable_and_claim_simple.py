...
def test_disable_before_start_time_and_reenable(
    vesting_simple, coin_a, accounts, chain, end_time
):
    vesting_simple.toggle_disable(accounts[1], {"from": accounts[0]})
    chain.sleep(end_time - chain.time())
    vesting_simple.toggle_disable(accounts[1], {"from": accounts[0]})
...
def test_disable_partially_ununclaimed(
    vesting_simple, coin_a, accounts, chain, start_time, end_time
):
...
def test_disable_multiple_partial(
    vesting_simple, coin_a, accounts, chain, start_time, end_time
):
    chain.sleep(start_time - chain.time() + 31337)
    vesting_simple.toggle_disable(accounts[1], {"from": accounts[0]})

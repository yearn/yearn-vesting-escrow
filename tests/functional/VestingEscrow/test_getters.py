...
def test_unclaimed_supply(chain, vesting, end_time):
    assert vesting.unclaimedSupply() == 0
    chain.sleep(end_time - chain.time())
    chain.mine()
    assert vesting.unclaimedSupply() == sum(amounts)
...
def test_unclaimed_of(chain, vesting, accounts, end_time):
    assert vesting.unclaimed(accounts[0]) == 0
    chain.sleep(end_time - chain.time())
    chain.mine()
    assert vesting.unclaimed(accounts[0]) == amounts[0]


def test_locked_of(chain, vesting, accounts, end_time):
    assert vesting.locked(accounts[0]) == amounts[0]
    chain.sleep(end_time - chain.time())
    chain.mine()
    assert vesting.locked(accounts[0]) == 0
